...
\begin{frame}{}
  \centering
  \vspace{1mm}
  \includegraphics[height=.82\textheight,keepaspectratio]{strategy-contoh.pdf}\\
  {\scriptsize Contoh interaksi antar-elemen strategi (kasus restoran cepat saji).}
\end{frame}
  ...
  \begin{columns}[c]
    ...
    \scalebox{0.85}{
      \begin{tikzpicture}[
        ...
        \node[phase, fill=orange!45, line width=0.7mm] at (90:2.6cm) {A};
      \end{tikzpicture}
    }
    \column{0.6\textwidth}
    ...
    \begin{itemize}\setlength{\itemsep}{1.0pt}
      ...
      \item \textbf{\textcolor{orange!80!black}{A---Visi Arsitektur}}
...
\begin{frame}{Cara Membaca Diagram Strategi}
  ...
  \begin{itemize}\setlength{\itemsep}{2.5pt}
    ...
    \item \textbf{Realisasi}: arah tindakan merealisasikan \emph{kebutuhan}
          (bungkus BI-FAST, AML otomatis, patuh SNAP).
    \item \textbf{Pengaruh}: kapabilitas \emph{memengaruhi} \emph{sasaran} \&
          \emph{prinsip} (tak langsung); modernisasi core memengaruhi \emph{biaya
          turun}.
    \item \textbf{Layanan}: kapabilitas \emph{melayani} value stream Eksekusi
          Transfer Dana.
  \end{itemize}
